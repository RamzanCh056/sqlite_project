...
class EvaluationUpdate extends StatefulWidget {
   EvaluationUpdate(this.id,this.courseName, this.difficultyLevel, this.assesmentType,this.dateTime, this.notes,{Key? key}) : super(key: key);
  int id;
  String courseName;
  ...
  @override
  State<EvaluationUpdate> createState() => _EvaluationUpdateAddState();
}

class _EvaluationUpdateAddState extends State<EvaluationUpdate> {
  // All journals
  ...
  void initState() {
    ...
    _assessmentType.text=widget.assesmentType;
    _dateTime.text = widget.dateTime;
    _difficultyLevel.text=widget.difficultyLevel;
  ...
  Future<void> _updateItem() async {
    await SQLHelper.updateItem(
        widget.id,  _courseName.text, _assessmentType.text ,  fDate!.millisecondsSinceEpoch ,_difficultyLevel.text,_notes.text  );
    _refreshJournals();
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
      content: Text('Successfully update  evaluation!'),
    ));
  }
          ...
          child: Padding(
            ...
            child: Form(
              ...
              child: Column(children: [
                CommonTextFieldWithTitle('Course Name', 'Enter course name',
                    _courseName ,
                        (val) {
                      if (val!.isEmpty) {
  ...
  selectDate(BuildContext context, int index) async {
    ...
    if (selectDate != null) {
      setState(() {
        if (index == 0) {
          _dateTime.text = DateFormat('dd/MM/yyyy KK:MM a').format(selectDate!);
         fDate = selectDate;
          // eta.text =
